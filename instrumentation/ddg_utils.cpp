...
//  std::map<CustomDDGNode*, std::vector<CustomDDGNode*>>::iterator it = graph.begin();
//  while(it != graph.end()) {
...
bool* isReachableByStore(std::vector<FlowWriteInstruction*>* From, Instruction* To, DominatorTree* DT, LoopInfo* LI, unsigned* ConsideredStores) {
  ...
  unsigned bb_threshold = NumberOfStores < BB_THRESHOLD ? NumberOfStores : BB_THRESHOLD;
  *ConsideredStores = bb_threshold;
  ...
  unsigned idx = 0;
  for (std::vector<FlowWriteInstruction*>::reverse_iterator it = From->rbegin(); it != From->rend(); it++) {
    FlowWriteInstruction* MyStore = *it;
    // TopNStores contains the last N stores, which are the ones that we check if are reachable.
    // These are put in reverse order, i.e., the position `0` (TopNstores[0]) is the last store that we met
    // (which is the last in the vector From)
  ...
  // We need the ExclusionSet to be complete, before startintg with the actual check loop
  for (int i = 0; i < bb_threshold; i++) {
    Instruction* FromInstruction = TopNstores[i]->I;
    if (TopNstores[i]->BB == To->getParent()) {
      // If the two BBs are the same, we discard this flow. It is not interesting since 
      // if we reach the BB we cover it
    ...
    if (FromInstruction != To) {
      bool r = isPotentiallyReachable(FromInstruction, To, &ExclusionSet, DT, LI);
