...
private:
	...
	void InsertDataFlow(Value* Operand, Value* Res) {
  	std::map<Value*, std::vector<Value*>>::iterator it = this->DataFlowTracker.begin();
  ...
  bool isSourceCodeVariable(Value* Variable) {
    std::map<Value*, std::vector<Value*>>::iterator it = this->DataFlowTracker.find(Variable);
    return it != this->DataFlowTracker.end();
  }


  bool isLLVMVariable(Value* Variable, std::map<Value*, Instruction*>* LLVMVariables) {
    std::map<Value*, Instruction*>::iterator it = LLVMVariables->find(Variable);
  ...
  // When we have `Store A, B`, we want to know that exactly B reperensents. In the default case, it is a source code variable and so we're done.
  // BUT, in many cases B could represent the field of a struct, or a location whithin a buffer. So, we need to recover what B represents to be 
  // more precise when we define the dependency relationship.
  void RetrieveAccessedVariable(Value* Variable, std::vector<Value*>* Flows, std::map<Value*, Instruction*>* LLVMVariables, Value** ActualSrcVariable) {

    if (isLLVMVariable(Variable, LLVMVariables)) {
      // If it is an LLVM variable (mostly for struct fields), we have it tracked down in the LLVMVariables list, so
      // we just need to parse the GEP inst
      ...
      // If it is not a GEP-defined llvm variable, we basically use the DataFlow Tracker, to retrieve the dependency of this variable.
      // The idea is that, if this llvm variable is not GEP-depending, it should be easier to retrieve what it does represent
      ...
      if (TmpFlow.size() == 1) {
        ...
        *ActualSrcVariable = TmpFlow[0];
        DEBUG(errs() << "[Warning] multiple flows for the same GEP access, choosing the first one\n");
      }
      else {
        return;
      } 
  ...
  bool runOnModule(Module &M) override {
    ...
    for (auto &F : M) {
      ...
      for (auto &BB : F) {
        BBCounter += 1;
				for (auto &I : BB) {
					// We track all variables "Alloca" derived and we add them to the RootNode
          ...
          if (Result and ! isa<CallInst>(I)) {              // We exclude CallInst, as they're managed separately (Not excluding them now, would introduce a double dependency leading to the same value)
            ...
            std::map<Function*, std::vector<Instruction*>>::iterator it = ReturnValues.find(CalledFunction);
            if (it != ReturnValues.end()) {
              std::vector<Instruction*> RetValsInstrs = it->second;
              for(std::vector<Instruction*>::iterator jt = RetValsInstrs.begin(); jt != RetValsInstrs.end(); jt++) {
                Instruction* In = *jt;
                ReturnInst* Ret = static_cast<ReturnInst*>(In);
                Value* RV = Ret->getReturnValue();
                CreateDataFlow(RV);
                InsertDataFlow(RV, Result);     // We indicate dependency between retval and call site
                Stores[RV].push_back(new FlowWriteInstruction(Ret->getParent(), Ret, declaration));
              }
            }
          }
#endif
					// We create the actual DDG depending on mem accesses and Call instructions
					if (auto ST = dyn_cast<StoreInst>(&I)) {
						Value* Variable = ST->getPointerOperand();      // Where we're writing
						Value* Access = ST->getValueOperand();          // What we're writing, this gives us the dependencies
          ...
          // If `Variable` does not directly represent a Src code variable, we fetch what it represents (e.g., the field of a struct)
            ...
            StoreType Type = declaration;                   // Usually we have `a = c + b`
						for (std::vector<Value*>::iterator it = Flows.begin(); it != Flows.end(); ++it) {
							Value* Dependency = *it;
              if (Dependency == Variable)                   // If we fall into `a += c + b`, we manage differently
                Type = modification;                        // Probably we dont need this distinction anymore, but keep it for future experiments

              // First we find the edges between the current store and the previous ones
              // (i.e., when we wrote into `c` and `b` if the current store is `a = c + b`)
              std::vector<FlowWriteInstruction*> AllStoresPerVariable = Stores[Dependency];
              unsigned ConsideredStores = 0;
              bool* ReachingStores = isReachableByStore(&AllStoresPerVariable, ST, &DT, &LI, &ConsideredStores);
            ...
            // Then we insert the new Store in our map that contains all the stores, so we build forward deps
            FlowWriteInstruction* MyStore = new FlowWriteInstruction(ST->getParent(), ST, Type);
            Stores[Variable].push_back(MyStore);

          }
          // Three major cases:
          // 1) a = foo(x)           => a depends on the result of foo() applied on x and x depends on its previous values and return value
          // 2) memcpy(src, dst, N)  => dst depends on src and N && the triple src, dst, N depends on their previous value (memcpy or any other API)
          // 3) foo(x, out_y, out_z) => out_x, out_y are writen within foo depending on x. Thus here the dependency is managed internally to the function when passing on it
            ...
            int argStart = 0;         // In some cases, we dont want to track dependencies for each argument.
                                      ...
                                      // case, we generate a FlowWriteInstruction object to save the fact that
                                      // `src` internal value has been modified according to `dst` and `n`
            ...
            if (FC == nullptr)
              ...
              switch (FC->getIntrinsicID()) { 
                case Intrinsic::memcpy: {
                  ...
                  if (Flows.size() != 0)
                    Variable = Flows[0];
                  MyStore = new FlowWriteInstruction(Call->getParent(), Call, declaration);
                  argStart = 1;
                  break;
                }
                ...
                default: {
                  //errs() << "Not implemented/interesting intrinsic for data flow\n";
                  break;
                }
              }
            }
      ...
      for (auto &BB : F) {
        ...
        Value* MapPtrIdx = IRB.CreateGEP(MapPtr, HashedLoc);
